def ingresar_datos():
    ...
    for i in range(num_restricciones):
        ...
        # Se registra la desigualdad tal como se ingresa
...
def convertir_a_forma_estandar(num_vars, num_restricciones, funcion_objetivo, restricciones, desigualdades):
    ...
    for i in range(num_restricciones):
        ...
        # Si es >= y el lado derecho es negativo, invertimos signos
        if desigualdades[i] == ">=":
            if restricciones[i][-1] < 0:
                restricciones[i] = [-x for x in restricciones[i]]  # Invertir signos de los coeficientes y lado derecho
...
def simplex():
    ...
    # Mostrar formulación inicial tal como se ingresó
